...
The generator selects a `/27` from `172.29.0.0/16`, removes every declared
overlap, then assigns one target and three to eight decoys to unique usable
addresses. A seed makes a failed round reproducible without exposing the answer
key to the student-facing brief. Production must additionally inspect live
Docker networks and fail closed before creating the scenario network.

Generate a development manifest with explicit known exclusions:
